...
abstract class Mesh {
  ...
  model.Frustum frustum = model.Frustum();

  void initialize(int width, int height) {
    this.width = width;
    this.height = height;

    rotationInc.setValues(0.6, 0.9, 0.2);

    aspectRatio = height / width;
    projectionMatrix = configureProjectionMatrix(fov, aspectRatio, znear, zfar);
    frustum.initialize(fov, znear, zfar);
  ...
  void update(Vector3 translation, Camera camera, double deltaTime) {
    ...
    for (Vector3 v in vertices) {
      ...
      // Multiply view matrix to transform to scene space
    ...
    for (Face face in faces) {
      ...
      // ----------------------------------------------------
      // Clipping into a polygon collection.
      // ----------------------------------------------------
      // Clip the polygon and returns a new polygon with potential new vertices
      Polygon polygon = Polygon.create(a, b, c);
